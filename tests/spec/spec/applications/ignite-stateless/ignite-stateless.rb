...
def callIgniteDeploymentTests
  ...
  describe 'Checking if it is possible to create a test table via API' do
    describe command("curl 'http://#{rest_api_host}:#{rest_nodeport}/ignite?cmd=qryfldexe&pageSize=10&cacheName=#{cacheName}&qry=CREATE+TABLE+PUBLIC.LS_TEST_TABLE(id+int,+name+varchar,+PRIMARY+KEY+(id))+WITH+\"CACHE_NAME=#{cacheName}-ddl\"'") do
      its(:stdout_as_json) { should include('successStatus' => 0) }
      its(:exit_status) { should eq 0 }
    end
  end
  ...
  describe 'Checking if it is possible to insert values into the test table via API' do
    describe command("curl 'http://#{rest_api_host}:#{rest_nodeport}/ignite?cmd=qryfldexe&pageSize=10&cacheName=#{cacheName}&qry=INSERT+INTO+PUBLIC.LS_TEST_TABLE(id,+name)+values+(1,+%27SUCCESS%27)'") do
      its(:stdout_as_json) { should include('successStatus' => 0) }
      its(:exit_status) { should eq 0 }
    end
  end
  ...
  describe 'Checking if it is possible to select values from the test table via API' do
    describe command("curl 'http://#{rest_api_host}:#{rest_nodeport}/ignite?cmd=qryfldexe&pageSize=10&cacheName=#{cacheName}&qry=SELECT+NAME+FROM+PUBLIC.LS_TEST_TABLE' | jq '.response.items'") do
  ...
  describe 'Checking if it is possible to select values from the test table using JDBC connection inside pods' do
    describe command("for pod in $(kubectl get pods --namespace=#{service_namespace} --no-headers --field-selector=status.phase=Running -o custom-columns=':metadata.name'); do kubectl exec $pod -n=ignite -- bash -c 'echo \"SELECT name FROM LS_TEST_TABLE;\" | $IGNITE_HOME/bin/sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ 2>&1'; done") do
  ...
  describe 'Checking if it is possible to drop the test table via API' do
    describe command("curl 'http://#{rest_api_host}:#{rest_nodeport}/ignite?cmd=qryfldexe&pageSize=10&cacheName=#{cacheName}&qry=DROP+TABLE+IF+EXISTS+PUBLIC.LS_TEST_TABLE'") do
      its(:stdout_as_json) { should include('successStatus' => 0) }
      its(:exit_status) { should eq 0 }
    end
  end
