#!/usr/bin/env bash
# Description: This script configures Hashicorp Vault to be used with LambdaStack
# You can find more information in LambdaStack documentation in HOWTO.md
...
function configure_kubernetes {
    ...
    if [ "${command_result[1]}" = "0" ] ; then
        ...
        if [ "$helm_custom_values_set_bool" = "true" ] ; then
          helm upgrade --install --wait -f /tmp/vault_helm_chart_values.yaml vault /tmp/v0.11.0.tar.gz --namespace vault
        else
          helm upgrade --install --wait vault /tmp/v0.11.0.tar.gz --namespace vault
        fi
        check_status $? "Vault Agent Helm Chart installed." "There was an error during installation of Vault Agent Helm Chart.";
    fi
}

function apply_lambdastack_vault_policies {
    log_and_print "Applying LambdaStack default Vault policies...";
    local vault_config_data_path="$1";
...
apply_lambdastack_vault_policies "$VAULT_CONFIG_DATA_PATH";
