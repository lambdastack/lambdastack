...
def createTable
  describe 'Checking if it is possible to create a test table' do
    describe command("echo \"CREATE TABLE LS_TEST_TABLE (id int, name varchar, PRIMARY KEY (id)) WITH \\\"CACHE_NAME=#{$cacheName}-ddl\\\";\" | /opt/ignite/bin/sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ 2>&1") do
      let(:disable_sudo) { false }
...
def insertValuesIntoTable
  describe 'Checking if it is possible to insert values into the test table' do
    describe command("echo \"INSERT INTO LS_TEST_TABLE(id, name) values (1, 'SUCCESS');\" | /opt/ignite/bin/sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ 2>&1") do
      let(:disable_sudo) { false }
...
def selectValuesFromTable
  describe 'Checking if it is possible to select values from the test table' do
    describe command("echo \"SELECT id, name FROM LS_TEST_TABLE;\" | /opt/ignite/bin/sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ 2>&1") do
      let(:disable_sudo) { false }
...
def getValueFromTableViaAPI
  describe 'Checking if it is possible to get values from table created with DDL statement ' do
    describe command("curl 'http://#{$ignite_rest_api_host}:#{$ignite_rest_api_port}/ignite?cmd=qryfldexe&pageSize=10&cacheName=#{$cacheName}-ddl&qry=select+name+from+LS_TEST_TABLE' | jq '.response.items'") do
...
def dropTable
  describe 'Checking if it is possible to drop the test table' do
    describe command("echo \"DROP TABLE IF EXISTS LS_TEST_TABLE;\" | /opt/ignite/bin/sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ 2>&1") do
      let(:disable_sudo) { false }
      its(:stdout) { should match /No rows affected/ }
      its(:stdout) { should_not match /Error/ }
      its(:exit_status) { should eq 0 }
